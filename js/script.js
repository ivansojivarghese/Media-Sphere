...
/*
  ...
  document.addEventListener("dblclick", (event) => { // double-click (by pointing device)
    ...
    function openProfile(p) {
      ...
      const profileBtn = document.querySelector("#infoContainer .head .profileBtn");
      profileBtn.classList.add("active");

      const profileWrapper = document.querySelector("#infoContainer .wrapper.profile");
    ...
    function seekForward(m) {
      ...
      if ((player && !player.isConnected) || !player) {
        if ((videoControls.classList.contains('visible') || m) && ((video.src !== "" && (video.currentTime < (video.duration - skipTime))) || (audioMode && (audio.currentTime < (audio.duration - skipTime)))) && !qualityBestChange && !qualityChange && !audioVideoAligning && seekAllow) {
    ...
    function seekBackward(m) {
      ...
      if ((player && !player.isConnected) || !player) {
        if ((videoControls.classList.contains('visible') || m) && ((video.src !== "" && (video.currentTime > (skipTime))) || (audioMode && (audio.currentTime > (skipTime)))) && !qualityBestChange && !qualityChange && !audioVideoAligning && seekAllow) {
            //backwardSkippedTime = 0;
            //seekBackwardTextSec.innerHTML = backwardSkippedTime;
...
/*
    ...
    */
    ...
    function updateVideoLoad() {
      const buffered = (!backgroundPlay && !audioMode) ? video.buffered : audio.buffered;
      const liveTime = (!backgroundPlay && !audioMode) ? video.currentTime : audio.currentTime;

      const MOD_BUFFER_THRESHOLD = (!backgroundPlay && !audioMode) ? BUFFER_THRESHOLD : BUFFER_THRESHOLD_AUDIO;
